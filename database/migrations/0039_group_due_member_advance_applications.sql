-- Group Labour Dues are one payable obligation. Advances owned by labourers in
-- the due's immutable member snapshot may therefore be applied to that group due.
-- Keep all balance, tenant and amount checks in the existing database guard.
...
BEGIN
  ...
  scope_is_eligible := target_advance.financial_scope_key = target_due.financial_scope_key;
  IF NOT scope_is_eligible AND target_due.recipient_scope = 'LABOUR_GROUP' AND target_advance.labourer_id IS NOT NULL THEN
    SELECT EXISTS (
      SELECT 1
      FROM labour_due_member_snapshots member
      WHERE member.workspace_id = NEW.workspace_id
        AND member.due_id = NEW.due_id
        AND member.labourer_id = target_advance.labourer_id
    ) OR EXISTS (
      SELECT 1
      FROM jsonb_array_elements(COALESCE(target_due.recipient_snapshot->'memberCalculationSnapshot', '[]'::jsonb)) member
      WHERE member->>'labourerId' = target_advance.labourer_id
