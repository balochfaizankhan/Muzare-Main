...
async function buildCleanupPreview(tx:DbTransaction,workspaceId:string,farmId:string,seasonId:string,targets:CleanupTarget[]){
  ...
  const targetPredicates=targets.map((target)=>and(
    ...
    eq(operationalRecords.id,target.id),
  ));
  ...
  const targetClientIds=records.map((row)=>row.clientRecordId);
  ...
  const dependencies:PreviewDependency[]=[
    ...
    {kind:"Labour earnings",count:linkedSourceRows.filter((row)=>row.entityType==="labourEarning"&&!targetClientIds.includes(row.clientRecordId)).length,classification:"WILL_UNLOCK",message:"Unselected earnings are preserved and returned to pending settlement."},
  ...
  const recordPreviews=records.map((row)=>{const rowDues=dues.filter((due)=>due.sourceRecordId===row.id||due.sourceClientRecordId===row.clientRecordId);const rowDueIds=rowDues.map((due)=>due.id);const rowPayments=paymentAllocations.filter((item)=>rowDueIds.includes(item.dueId));const rowAdvances=advanceApplications.filter((item)=>rowDueIds.includes(item.dueId));const rowVoucherIds=new Set(rowPayments.map((item)=>item.voucherId));const rowVouchers=vouchers.filter((item)=>rowVoucherIds.has(item.id)||rowDueIds.includes(item.linkedDueId??""));const rowAccounting=accountingEntries.filter((item)=>rowDueIds.includes(item.dueId??"")||rowVoucherIds.has(item.voucherId??""));const rowBlocked=rowVouchers.some((voucher)=>sharedVoucherIds.includes(voucher.id));const rowCascade=rowDues.length+rowPayments.length+rowAdvances.length+rowVouchers.length+rowAccounting.length>0;return {entityType:row.entityType==="labourEarning"?"EARNING":"SETTLEMENT",id:row.id,clientId:row.clientRecordId,reference:row.entityType==="labourEarning"?text(row.payload.reference)||`LE-${row.clientRecordId.slice(0,8).toUpperCase()}`:text(row.payload.settlementNumber)||row.clientRecordId,recipient:text(row.payload.groupName)||text(row.payload.labourGroupName)||text(row.payload.foremanName)||null,status:text(row.payload.status)||"unknown",period:{from:text(row.payload.fromDate)||text(row.payload.date),to:text(row.payload.toDate)||text(row.payload.date)},amount:row.entityType==="labourEarning"?number(row.payload.amount):number(row.payload.grossWages||row.payload.totalEarned||row.payload.expenseAmount),classification:rowBlocked?"BLOCKED":rowCascade?"CASCADE_REQUIRED":text(row.payload.status)==="voided"?"ELIGIBLE":"REQUIRES_REVIEW",linkedDue:rowDues[0]?.dueNumber??null,counts:{dues:rowDues.length,paymentVouchers:rowVouchers.length,paymentAllocations:rowPayments.length,advanceApplications:rowAdvances.length,accountingEntries:rowAccounting.length},blockers:rowBlocked?["A linked payment voucher is shared with an unrelated due."]:[]};});
  return {targets:recordPreviews,expandedSettlements:settlements.map((row)=>({id:row.id,reference:text(row.payload.settlementNumber)||row.clientRecordId})),dependencies,counts:{selected:records.length,settlements:settlements.length,earnings:selectedEarnings.length,dues:dues.length,paymentVouchers:vouchers.length+legacyVouchers.length,paymentAllocations:paymentAllocations.length,advanceApplications:advanceApplications.length+oldAllocations.length,accountEffects:accountEntries.length,accountingEntries:accountingEntries.length,attendanceUnlocks:linkedSourceRows.filter((row)=>row.entityType==="attendance").length,earningUnlocks:linkedSourceRows.filter((row)=>row.entityType==="labourEarning"&&!targetClientIds.includes(row.clientRecordId)).length},totalGrossAmount:recordPreviews.reduce((sum,row)=>sum+row.amount,0),totalFinancialAmount:vouchers.reduce((sum,row)=>sum+number(row.paymentAmount),0),financialEffects,requiresCascade:dues.length+paymentAllocations.length+advanceApplications.length+vouchers.length+legacyVouchers.length+accountEntries.length+accountingEntries.length>0,blocked,blockedReasons:sharedVoucherIds.length?["A linked payment voucher is shared with an unrelated due."]:[],internal:{records,selectedEarnings,settlements,dues,paymentAllocations,advanceApplications,vouchers,legacyVouchers,accountingEntries,accountEntries,partnerEffects,linkedSourceRows,oldAllocations,createRequests}};
}
...
export async function labourReconciliationRoutes(app:FastifyInstance):Promise<void>{
  ...
  // @ts-expect-error The combined validation guard guarantees the selected failure object exists.
  app.post("/v1/workspace/:workspaceId/labour-reconciliation/cleanup/preview",{preHandler:requireUser},async(request,reply)=>{const params=paramsSchema.safeParse(request.params);const body=previewSchema.safeParse(request.body);if(!params.success||!body.success)return reply.code(400).send({message:"Select labour data to preview.",details:body.success?params.error.flatten():body.error.flatten()});if(!(await requireScope(request,reply,params.data.workspaceId,body.data.farmId,body.data.seasonId,"delete")))return;try{const preview=await db.transaction((tx)=>buildCleanupPreview(tx,params.data.workspaceId,body.data.farmId,body.data.seasonId,body.data.targets));if(preview.targets.length!==new Set(body.data.targets.map((item)=>item.id)).size)return reply.code(409).send({message:"One or more selected settlements could not be found in this workspace, farm, and season."});const {internal:_,...safe}=preview;return{preview:safe};}catch(error){request.log.error({err:error,workspaceId:params.data.workspaceId,targetIds:body.data.targets.map((item)=>item.id)},"Settlement deletion preview failed");return reply.code(500).send({message:"Deletion preview could not be generated.",details:error instanceof Error?error.message:"Unknown preview error"});}});
  app.post("/v1/workspace/:workspaceId/labour-reconciliation/cleanup/execute",{preHandler:requireUser},async(request,reply)=>{const params=paramsSchema.safeParse(request.params);const body=executeSchema.safeParse(request.body);if(!params.success||!body.success)return reply.code(400).send({message:"A valid cleanup confirmation is required."});const {workspaceId}=params.data;const input=body.data;if(!(await requireScope(request,reply,workspaceId,input.farmId,input.seasonId,"delete")))return;try{const result=await db.transaction(async(tx)=>{
    ...
    for(const row of internal.linkedSourceRows){if(targetIds.has(row.id))continue;const next={...row.payload,linkedSettlementId:null,settlementDate:null,updatedAt:now.toISOString(),...(row.entityType==="labourEarning"?{status:"pending_settlement"}: {})};await tx.update(operationalRecords).set({payload:next,clientUpdatedAt:now,updatedAt:now}).where(eq(operationalRecords.id,row.id));}
    const deletedRows=[...new Map([...internal.selectedEarnings,...internal.settlements].filter((row)=>targetIds.has(row.id)||row.entityType==="labourWageSettlement").map((row)=>[row.id,row])).values()];
