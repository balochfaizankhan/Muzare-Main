  ...
  app.get("/v1/workspace/:workspaceId/advance/report", { preHandler: requireUser }, async (request, reply) => {
    ...
    const records = ledger.rows.flatMap((row) => {
      ...
      return [{
        ...
        recipientDisplayName: financial?.recipientDisplayName ?? financial?.recipientName ?? labourName,
        ...
        accountName: financial?.paymentSourceDisplayName ?? financial?.fundingAccountName ?? row.accountName ?? accountById.get(row.accountId ?? "") ?? "",
        paymentSourceDisplayName: financial?.paymentSourceDisplayName ?? financial?.fundingAccountName ?? row.accountName ?? accountById.get(row.accountId ?? "") ?? "",
    ...
    for (const row of financials.advancePositions) {
      ...
      records.push({
        ...
        labourName: row.recipientDisplayName ?? row.labourerName ?? row.labourGroupName ?? row.recipientName,
        recipientType: row.recipientScope,
        recipientDisplayName: row.recipientDisplayName ?? row.recipientName,
        date: row.advanceDate,
        amount: row.originalAmount,
        notes: row.description,
        accountId: row.fundingAccountId ?? "",
        accountName: row.paymentSourceDisplayName ?? row.fundingAccountName ?? accountById.get(row.fundingAccountId ?? "") ?? "",
        paymentSourceDisplayName: row.paymentSourceDisplayName ?? row.fundingAccountName ?? accountById.get(row.fundingAccountId ?? "") ?? "",
