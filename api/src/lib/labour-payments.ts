      ...
      if (advanceRecord) {
        ...
        if (advanceAmount > 0) {
          [advanceVoucher] = await tx.insert(labourPaymentVouchers).values({
            ...
            recipientSnapshot: { labourGroupName: advancePayload.labourGroupName ?? null, sourceAccountName: advancePayload.sourceAccountName ?? null, legacyOperationalRecord: true },
