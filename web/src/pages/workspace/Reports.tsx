...
export function Reports() {
  ...
  const exportPartnerPosition = () => downloadCsv("partner-position.csv", [
    [t("reportsPage.partner"), t("reportsPage.openingBalance"), t("reportsPage.capitalInjected"), "Purchase vouchers", "Total labour advances paid", "Settled through wage settlements", "Outstanding labour advances", "Labour settlements", t("reportsPage.transfersOut"), t("reportsPage.transfersIn"), t("reportsPage.moneyReturned"), t("reportsPage.adjustments"), t("reportsPage.currentPartnerBalance")],
    ...partnerLiabilityPositions.map((item) => [item.name, item.openingBalance, item.capitalInjected, item.purchaseVouchersPaid, item.totalLabourAdvancesPaid, Math.max(item.totalLabourAdvancesPaid - item.outstandingLabourAdvances, 0), item.outstandingLabourAdvances, item.labourWageSettlements, item.transfersOut, item.transfersIn, item.moneyReturned, item.adjustments, item.currentPartnerBalance]),
  ]);
    ...
    <main className="subpage module-workspace reports-page">
      ...
      {report === "partner-position" && <>
        ...
        {views["partner-position"] === "position" && <ReportShell title={t("reportsPage.partnerPositionTitle")} rangeLabel={rangeLabel} sectionId="partner-position" onPrint={() => printSection("partner-position")} onExport={exportPartnerPosition}>
          <ReportTable empty={t("reportsPage.noRecords")} columns={[t("reportsPage.partner"), t("reportsPage.capitalInjected"), "Purchase vouchers", "Business funds net", "Outstanding labour advances", "Labour settlements", t("reportsPage.currentPartnerBalance")]} rows={partnerLiabilityPositions.map((item) => ({ id: item.key, title: item.name, value: money(item.currentPartnerBalance), meta: getPartnerBalanceState(item.currentPartnerBalance) === "partner_holds_business_money" ? t("reportsPage.partnerHoldsBusinessMoney") : t("reportsPage.farmOwesPartner"), cells: [item.name, money(item.capitalInjected), money(item.purchaseVouchersPaid), money(item.businessFundsNet), money(item.outstandingLabourAdvances), money(item.labourWageSettlements), money(item.currentPartnerBalance)], details: [[t("reportsPage.openingBalance"), money(item.openingBalance)], [t("reportsPage.adjustments"), money(item.adjustments)], ["Business funds given", money(item.transfersOut)], ["Business funds received", money(item.transfersIn)], ["Business funds net", money(item.businessFundsNet)], ["Total labour advances paid", money(item.totalLabourAdvancesPaid)], ["Less: settled through wage settlements", money(Math.max(item.totalLabourAdvancesPaid - item.outstandingLabourAdvances, 0))], ["Outstanding labour advances", money(item.outstandingLabourAdvances)], [t("reportsPage.moneyReturned"), money(item.moneyReturned)]] }))} />
        </ReportShell>}
