...
function AdvancesView({ labourers, groups, accounts, canManage, token, workspaceId, farmId, seasonId, onSaved, onError }: { labourers: Labourer[]; groups: LabourGroup[]; accounts: Account[]; canManage: boolean; token: string; workspaceId: string; farmId: string; seasonId: string; onSaved: (message: string) => Promise<void>; onError: (message: string) => void }) {
  ...
  const [selectedAdvance,setSelectedAdvance]=useState<LabourAdvancePosition|null>(null); const [showForm,setShowForm]=useState(false); const [scope,setScope]=useState<LabourRecipientScope>("INDIVIDUAL"); const [labourerId,setLabourerId]=useState(""); const [groupId,setGroupId]=useState(""); const [receivedBy,setReceivedBy]=useState(""); const [reference,setReference]=useState(""); const [recipientName,setRecipientName]=useState(""); const [date,setDate]=useState(today()); const [amount,setAmount]=useState(""); const [accountId,setAccountId]=useState(""); const [method,setMethod]=useState("Cash"); const [description,setDescription]=useState(""); const [transactionReference,setTransactionReference]=useState(""); const [saving,setSaving]=useState(false);
  ...
  const recipientGroups=useMemo(()=>Array.from(rows.reduce((map,advance)=>{const key=`${advance.recipientScope}:${advance.financialOwnerId??advance.financialOwnerName??"unavailable"}`;const current=map.get(key)??{key,name:advance.financialOwnerName??"Recipient unavailable",scope:scopeCopy(advance),count:0,outstanding:0,reviewRequired:false,receivers:new Set<string>()};current.count+=1;current.outstanding+=advance.outstandingAmount;current.reviewRequired ||= advance.reviewRequired;if(advance.receivedByName)current.receivers.add(advance.receivedByName);else if(advance.recipientScope==="INDIVIDUAL"&&advance.financialOwnerName)current.receivers.add(advance.financialOwnerName);map.set(key,current);return map;},new Map<string,{key:string;name:string;scope:string;count:number;outstanding:number;reviewRequired:boolean;receivers:Set<string>}>()).values()),[rows]);
  const activeLabourers=useMemo(()=>labourers.filter((item)=>item.active!==false),[labourers]);
  const groupOptions=useMemo(()=>groups.map((item)=>({id:item.id,name:item.name,active:true} as Labourer)),[groups]);
  const accountOptions=useMemo(()=>accounts.map((item)=>({id:item.id,name:item.name,active:true} as Labourer)),[accounts]);
  const selectedOwner=scope==="INDIVIDUAL"?labourers.find((item)=>item.id===labourerId)?.name:scope==="LABOUR_GROUP"?groups.find((item)=>item.id===groupId)?.name:recipientName||reference;
  const selectedReceiver=scope==="INDIVIDUAL"?selectedOwner:scope==="LABOUR_GROUP"?receivedBy:"";
  const selectedAccount=accounts.find((item)=>item.id===accountId)?.name;
  const formValid=Boolean(date&&Number(amount)>0&&accountId&&description.trim()&&(scope==="INDIVIDUAL"?labourerId:scope==="LABOUR_GROUP"?groupId&&receivedBy:reference));
  return <><section className="record-panel workforce-payments-panel workforce-advances-panel"><header className="workforce-payments-panel__header workforce-advances-header"><div><h2>Outstanding Advances</h2><p>{pageInfo.totalCount?`${rows.length} of ${pageInfo.totalCount} advances loaded`:"Advance balances by financial owner"}</p></div>{canManage?<button className="primary-action workforce-record-advance" type="button" onClick={()=>setShowForm(true)}><Plus size={16}/> Record advance</button>:null}</header>
    ...
    <div className="workforce-advance-toolbar"><label className="workforce-payments-search"><Search size={16}/><input value={searchInput} onChange={(event)=>setSearchInput(event.target.value)} placeholder="Search recipient, group, voucher, or account"/></label><div className="workforce-advance-view-toggle" role="group" aria-label="Advance display"><button type="button" aria-pressed={groupMode} onClick={()=>setGroupMode(true)}>Grouped</button><button type="button" aria-pressed={!groupMode} onClick={()=>setGroupMode(false)}>Vouchers</button></div><button className="secondary-action" type="button" aria-expanded={showFilters} onClick={()=>setShowFilters((value)=>!value)}>Filters</button></div>
    ...
    {initialLoading&&!rows.length?<div className="workforce-advance-skeletons" aria-label="Loading outstanding advances">{Array.from({length:4},(_,index)=><div key={index} className="workforce-advance-skeleton"/>)}</div>:loadError&&!rows.length?<div className="workforce-payments-empty"><p>{loadError}</p><button className="secondary-action" type="button" onClick={()=>void loadPage(1,false)}>Retry</button></div>:!rows.length?<p className="workforce-payments-empty">No outstanding advances match these filters.</p>:groupMode?<div className="workforce-advance-recipient-list">{recipientGroups.map((group)=>{const receivers=[...group.receivers];return <article key={group.key}><div><strong>{group.name}</strong><span>{receivers.length?`Received by ${receivers[0]}${receivers.length>1?` +${receivers.length-1}`:""}`:"Receiver unavailable"}</span></div><div><b>{money(group.outstanding)} outstanding</b><span>{group.count} loaded {group.count===1?"advance":"advances"}{group.reviewRequired?" · Review required":""}</span></div><div className="workforce-advance-group-actions"><button className="workforce-advance-link" type="button" disabled={group.name==="Recipient unavailable"} onClick={()=>{setGroupMode(false);setSearchInput(group.name);}}>View advances</button></div></article>})}</div>:<div className="workforce-advance-position-list">{rows.map((advance)=>{const receiver=advance.receivedByName||(advance.recipientScope==="INDIVIDUAL"?advance.financialOwnerName:null);return <article key={advance.id} className="workforce-advance-card" tabIndex={0} role="button" onClick={()=>setSelectedAdvance(advance)} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();setSelectedAdvance(advance);}}}><header><div><span>{receiver??"Receiver unavailable"}</span><small>{advance.recipientScope==="LABOUR_GROUP"?`Received by · for ${advance.financialOwnerName??"Owner unavailable"}`:"Individual labourer"}{advance.reviewRequired?" · Review required":""}</small></div><em className={`workforce-payment-status status-${advance.advanceStatus.toLowerCase()}`}>{statusLabel(advance.advanceStatus)}</em></header><div className="workforce-advance-card__reference"><span>{advance.displayVoucherNumber}</span><time>{formatDate(advance.voucherDate)}</time></div><div className="workforce-advance-card__money"><div><span>Outstanding</span><strong>{money(advance.outstandingAmount)}</strong></div><small>Original {money(advance.originalAmount)}{advance.appliedAmount>0?` · Applied ${money(advance.appliedAmount)}`:""}{advance.refundedAmount>0?` · Recovered ${money(advance.refundedAmount)}`:""}</small></div><footer><span>{advance.paymentAccountName??"Account unavailable · Legacy record"}</span><div><button type="button" className="workforce-advance-link" onClick={(event)=>{event.stopPropagation();setSelectedAdvance(advance);}}>Details</button>{canManage&&!advance.readOnlyLegacy&&advance.status==="POSTED"?<button type="button" className="secondary-action workforce-advance-recover" onClick={(event)=>{event.stopPropagation();setRefundAdvance(advance);setRefundAmount(String(advance.outstandingAmount));}}>Recover</button>:null}</div></footer></article>})}</div>}
  ...
  {showForm?<div className="worker-dialog-backdrop workforce-payment-review-backdrop" role="presentation" onClick={()=>setShowForm(false)}><form className="workforce-payment-review workforce-advance-entry-sheet" onSubmit={(event)=>void submit(event)} onClick={(event)=>event.stopPropagation()}><header><div><span>Labour payment voucher</span><h2>Record advance</h2><p>Record money paid before final settlement</p></div><button type="button" onClick={()=>setShowForm(false)} aria-label="Close"><X size={18}/></button></header><div className="workforce-payment-review__body workforce-advance-entry-body"><section><h3><span>1</span> Recipient</h3><div className="workforce-payment-form"><label><span>Recipient scope</span><select value={scope} onChange={(event)=>setScope(event.target.value as LabourRecipientScope)}>{scopeOptions.slice(0,5).map((item)=><option key={item.value} value={item.value}>{item.label}</option>)}</select></label>{scope==="INDIVIDUAL"?<label><span>Labourer</span><LabourSelectCombobox ariaLabel="Labourer" options={activeLabourers} value={labourerId} onChange={setLabourerId} placeholder="Search active labourers" noResultsLabel="No active labourers found"/></label>:null}{scope==="LABOUR_GROUP"?<><label><span>Labour group</span><LabourSelectCombobox ariaLabel="Labour group" options={groupOptions} value={groupId} onChange={setGroupId} placeholder="Search labour groups"/></label><label><span>Received by</span><input required value={receivedBy} onChange={(event)=>setReceivedBy(event.target.value)} placeholder="Leader or representative name"/></label></>:null}{!["INDIVIDUAL","LABOUR_GROUP"].includes(scope)?<><label><span>Recipient name</span><input value={recipientName} onChange={(event)=>setRecipientName(event.target.value)}/></label><label><span>Stable reference</span><input required value={reference} onChange={(event)=>setReference(event.target.value)}/></label></>:null}</div></section><section><h3><span>2</span> Payment</h3><div className="workforce-payment-form"><label><span>Date</span><input required type="date" value={date} onChange={(event)=>setDate(event.target.value)}/></label><label><span>Amount (SAR)</span><input required min="0.01" step="0.01" type="number" value={amount} onChange={(event)=>setAmount(event.target.value)}/></label><label><span>Paid from account</span><LabourSelectCombobox ariaLabel="Paid from account" options={accountOptions} value={accountId} onChange={setAccountId} placeholder="Search payment accounts"/></label><label><span>Method</span><select value={method} onChange={(event)=>setMethod(event.target.value)}><option>Cash</option><option>Bank Transfer</option><option>Other</option></select></label></div></section><section><h3><span>3</span> Details</h3><div className="workforce-payment-form"><label className="is-full"><span>Description</span><input required value={description} onChange={(event)=>setDescription(event.target.value)} placeholder="Purpose of this advance"/></label><label className="is-full"><span>Note / reference</span><input value={transactionReference} onChange={(event)=>setTransactionReference(event.target.value)} placeholder="Optional"/></label></div></section>{selectedOwner&&Number(amount)>0&&selectedAccount?<section className="workforce-advance-preview"><h3><span>4</span> Preview</h3><dl><div><dt>For</dt><dd>{selectedOwner}</dd></div>{selectedReceiver&&selectedReceiver!==selectedOwner?<div><dt>Received by</dt><dd>{selectedReceiver}</dd></div>:null}<div><dt>Amount</dt><dd>{money(Number(amount))}</dd></div><div><dt>Paid from</dt><dd>{selectedAccount}</dd></div><div><dt>Method</dt><dd>{method}</dd></div></dl></section>:null}</div><footer><div className="workforce-payment-review__actions"><button className="secondary-action" type="button" onClick={()=>setShowForm(false)}>Cancel</button><button className="primary-action" disabled={saving||!formValid} type="submit">{saving?"Posting…":"Post advance"}</button></div></footer></form></div>:null}
