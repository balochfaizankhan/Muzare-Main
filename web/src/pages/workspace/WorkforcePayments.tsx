...
export function WorkforcePaymentsPage() {
  ...
  return <div className="workforce-payments-page">
    ...
    {view === "advances" ? <AdvancesView advances={advances} labourers={labourers} groups={groups} accounts={accounts} canManage={canManage} token={token ?? ""} workspaceId={workspaceId} farmId={farmId} seasonId={seasonId} onSaved={async (message) => { setSuccess(message); await refresh(); }} onError={setError} /> : null}
...
function AdvancesView({ advances, labourers, groups, accounts, canManage, token, workspaceId, farmId, seasonId, onSaved, onError }: { advances: LabourAdvancePosition[]; labourers: Labourer[]; groups: LabourGroup[]; accounts: Account[]; canManage: boolean; token: string; workspaceId: string; farmId: string; seasonId: string; onSaved: (message: string) => Promise<void>; onError: (message: string) => void }) {
  const idempotencyKey = useRef(uuid());
  ...
  return <><section className="record-panel workforce-payments-panel"><header className="workforce-payments-panel__header"><div><h2>Outstanding Advances</h2><p>Original, applied, refunded, and remaining advance balances.</p></div>{canManage ? <button className="secondary-action" type="button" onClick={() => setShowForm((value) => !value)}><Plus size={16} /> Record advance</button> : null}</header>
    ...
    {!advances.length ? <p className="workforce-payments-empty">No labour advances are registered.</p> : <div className="workforce-advance-position-list">{advances.map((advance) => <article key={advance.id}><header><strong>{advance.voucherNumber}</strong><em className={`workforce-payment-status status-${advance.advanceStatus.toLowerCase()}`}>{statusLabel(advance.advanceStatus)}</em></header><p>{String(advance.recipientSnapshot.labourerName ?? advance.recipientSnapshot.labourGroupName ?? advance.recipientSnapshot.manualRecipientName ?? scopeLabel(advance.recipientScope))}</p><div><span>Original <b>{money(advance.originalAmount)}</b></span><span>Applied <b>{money(advance.appliedAmount)}</b></span><span>Refunded <b>{money(advance.refundedAmount)}</b></span><span className="is-outstanding">Outstanding <b>{money(advance.outstandingAmount)}</b></span></div>{canManage && advance.status === "POSTED" && advance.outstandingAmount > 0 ? <button className="secondary-action" type="button" onClick={() => { setRefundAdvance(advance); setRefundAmount(String(advance.outstandingAmount)); }}>Record recovery</button> : null}</article>)}</div>}
  </section></>;
}
