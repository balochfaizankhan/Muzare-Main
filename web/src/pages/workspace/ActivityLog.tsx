...
import { loadWorkspaceActivity, formatWorkspaceActivityDateTime, type WorkspaceActivityItem, type WorkspaceActivityModule } from "../../lib/workspaceActivity";
              ...
              {filteredActivities.map((activity) => {
                ...
                const content = (
                  <>
                    ...
                    {activity.path ? <ChevronRight size={16} className="activity-log-item__chevron" /> : null}
                  </>
                );
                return activity.path ? (
                  <Link to={activity.path} className="dashboard-activity-item activity-log-item" key={activity.id}>
                    {content}
                  </Link>
                ) : (
                  <article className="dashboard-activity-item activity-log-item activity-log-item--static" key={activity.id}>
                    {content}
                  </article>
                );
              })}
