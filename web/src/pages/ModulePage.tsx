...
const compactAdvance = (amount: number) => new Intl.NumberFormat("en", { maximumFractionDigits: 0 }).format(amount);
...
function AttendanceRegister({ data, syncStatus, totalPresent, totalHalf, totalAbsent, totalAdvance, totalWage, workerAdvance, dailyAdvance, dailyStatus, onClose, onCsv }: {
  ...
  return <section className="attendance-register-preview">
    ...
    <div className="register-table-wrap"><table className="attendance-register-table">
      <thead><tr><th>#</th><th>Labour Name</th><th>P</th><th>1/2</th><th>A</th><th>Adv (SAR)</th>{data.dates.map((date) => <th key={date}>{compactDate(date)}</th>)}</tr></thead>
      <tbody>{data.summaries.map((summary, index) => <tr key={summary.id}><td>{index + 1}</td><th>{summary.name}</th><td>{summary.presentDays}</td><td>{summary.halfDays}</td><td>{summary.absentDays}</td><td>{compactAdvance(workerAdvance(summary.id))}</td>
      ...
      <tfoot><tr><th colSpan={2}>Grand Total</th><th>{totalPresent}</th><th>{totalHalf}</th><th>{totalAbsent}</th><th>{compactAdvance(totalAdvance)}</th><th colSpan={data.dates.length}></th></tr>
